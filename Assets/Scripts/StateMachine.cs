...
public abstract class StateMachine<T> : MonoBehaviour where T : struct, IConvertible  {
	...
	static void DoNothingControlCollider(ControllerColliderHit other)
	{
	}
	...
	public Action<ControllerColliderHit> DoOnControllerColliderHit = DoNothingControlCollider;
	...
	public T CurrentState
	{
		...
		set
		{
            ...
            {
                StateChanged(this, new StateChangedEventArgs<T>(_currentState, value));
            }
	...
	{
		// Since we're switching to a new state, we need to call the ExitSate() of the current state
		if(ExitState != null)
		{
		...
		DoOnControllerColliderHit = ConfigureDelegate<Action<ControllerColliderHit>>("OnControllerColliderHit", DoNothingControlCollider);
	...
	K ConfigureDelegate<K>(string methodRoot, K Default) where K : class
	{
		var mtd = GetType().GetMethod(_currentState.ToString() + "_" + methodRoot, BindingFlags.Instance 
			| BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.InvokeMethod);
		
		if(mtd != null)
		{
			return Delegate.CreateDelegate(typeof(K), this, mtd) as K;
		}
