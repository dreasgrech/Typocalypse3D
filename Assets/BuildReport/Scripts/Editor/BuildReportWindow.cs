	...
	void OnGUI()
	{
		...
		if ((_selectedCategoryIdx == USED_ASSETS_IDX && _buildInfo.HasUsedAssets) || (_selectedCategoryIdx == UNUSED_ASSETS_IDX && _buildInfo.HasUnusedAssets))
		{
			...
			GUILayout.BeginHorizontal();
				...
				if (GUILayout.Button("Previous") && (viewOffset - BuildReportTool.Options.AssetListPaginationLength >= 0))
				{
				...
				GUILayout.Label("Viewing " + (viewOffset + (len > 0 ? 1 : 0)) + " - " + len + " of " + assetListToUse.Length);

				if (GUILayout.Button("Next") && (viewOffset + BuildReportTool.Options.AssetListPaginationLength < assetListToUse.Length))
				{
					assetListUsed.SetViewOffsetForDisplayedList(FileFilterGroupToUse, viewOffset + BuildReportTool.Options.AssetListPaginationLength);
				}
