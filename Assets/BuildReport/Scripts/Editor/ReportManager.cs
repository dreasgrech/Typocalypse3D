...
public partial class ReportManager
{
	...
	static bool IsFileInAPath(string filepath, string pathToCheck)
	{
		return filepath.ToLower().IndexOf(pathToCheck) != -1;
	}

	static bool IsFileOfType(string filepath, string typeExtenstion)
	{
	...
	static BuildReportTool.SizePart[] GetAllUnusedAssets(string[] scenesIncludedInProject, BuildReportTool.SizePart[] scriptDLLs, string projectAssetsPath, bool includeSvn, bool includeGit, BuildPlatform buildPlatform, bool includeUnusedPrefabs, List<BuildReportTool.SizePart> inOutAllUsedAssets)
	{
		...
		for (int assetIdx = 0; assetIdx < allAssets.Length; ++assetIdx)
		{
			...
			if (IsFileOfType(currentAsset, ".meta") || IsFileOfType(currentAsset, ".mask") || IsFileInAPath(currentAsset, "/resources/"))
			{
				continue;
			}

			// include version control files only if requested to do so
			if (!includeSvn && IsFileInAPath(currentAsset, "/.svn/"))
			{
				continue;
			}
			if (!includeGit && IsFileInAPath(currentAsset, "/.git/"))
			{
				continue;
			}
			...
			if (IsFileOfType(currentAsset, ".dll"))
			{
...
*/
			...
			switch (buildPlatform)
			{
				case BuildPlatform.Android:
					...
					if (IsFileInAPath(currentAsset, "assets/plugins/android/") && (IsFileOfType(currentAsset, ".jar") || IsFileOfType(currentAsset, ".so")))
					{
				...
				case BuildPlatform.iOS:
					if (IsFileOfType(currentAsset, ".a") || IsFileOfType(currentAsset, ".m") || IsFileOfType(currentAsset, ".mm") || IsFileOfType(currentAsset, ".c") || IsFileOfType(currentAsset, ".cpp"))
					{
						// any .a, .m, .mm, .c, or .cpp files inside Assets/Plugins/iOS are automatically symlinked/used
						if (IsFileInAPath(currentAsset, "assets/plugins/ios/"))
						{
							inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
						}
				...
				case BuildPlatform.MacOSX:
					...
					if (IsFileInAPath(currentAsset, "assets/plugins/") && IsFileOfType(currentAsset, ".bundle"))
					{
						inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
						continue;
					}
				...
				case BuildPlatform.Windows32:
					if (IsFileOfType(currentAsset, ".dll"))
					{
						if (IsFileInAPath(currentAsset, "assets/plugins/x86/"))
						{
							inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
							continue;
						}
						// Unity only makes use of Assets/Plugins/ if Assets/Plugins/x86/ does not exist
						else if (IsFileInAPath(currentAsset, "assets/plugins/") && !has32BitPluginsFolder)
						{
							inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
							continue;
						}
				...
				case BuildPlatform.Windows64:
					if (IsFileOfType(currentAsset, ".dll"))
					{
						if (IsFileInAPath(currentAsset, "assets/plugins/x86_64/"))
						{
							inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
							continue;
						}
						// Unity only makes use of Assets/Plugins/ if Assets/Plugins/x86_64/ does not exist
						else if (IsFileInAPath(currentAsset, "assets/plugins/") && !has64BitPluginsFolder)
						{
							inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
							continue;
						}
				...
				case BuildPlatform.Linux32:
					if (IsFileOfType(currentAsset, ".so"))
					{
						if (IsFileInAPath(currentAsset, "assets/plugins/x86/"))
						{
							inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
				...
				case BuildPlatform.LinuxUniversal:
					if (IsFileOfType(currentAsset, ".so"))
					{
						if (IsFileInAPath(currentAsset, "assets/plugins/x86/") || IsFileInAPath(currentAsset, "assets/plugins/x86_64/"))
						{
							inOutAllUsedAssets.Add(BuildReportTool.Util.CreateSizePartFromFile(currentAsset, fullAssetPath));
							continue;
						}
			...
			if (!includeUnusedPrefabs && IsFileOfType(currentAsset, ".prefab"))
			{
				continue;
			}
